# Source snapshot: pre-spec-instructions

Source: https://docs.google.com/document/d/1AqbEu42Uoa8b5x6REb8uLynOxSxSH9RsrzgAkQ96VE4
Captured: 2026-09-24
...
Owner: Lyle / ChatGPT-assisted
Change Summary: Added an explicit consolidated pre-historic reconstruction route based on the Filters retrospective workflow, distinguished it from ordinary meeting-by-meeting retrospective ingestion, standardized approval-gated reconciliation passes, and added current model-selection guidance for large-corpus ingestion.


Purpose: Define how Feature Working Model / Pre-Spec artifacts are initialized, maintained, reconciled after transcript ingestion, used to identify and curate Implementation Concepts, used to draft concept-level implementation instructions, used to capture Pre Spec Implementation Questions, aligned with Agents Docs when implementation questions arise, reconstructed for retrospective lab series, and shaped toward implementable Feature Spec Sheets.
...
- Agents Docs / implementation implications;
...
Model selection:
- Recommended model: GPT-5.6 Sol with xhigh reasoning for the full-corpus ingestion and first cross-transcript reconciliation.
- Fallback: GPT-5.6 Sol with high reasoning when xhigh is unavailable or turnaround time materially outweighs the incremental review depth.
- Do not use low, light, or routine/default reasoning for the authoritative consolidated ingestion pass.
- After the consolidated product model is approved, high reasoning is generally sufficient for focused cleanup, implementation-question drafting, and spec conversion. Return to xhigh when resolving material conflicts, reversals, or cross-document ambiguity.
- Treat the named model as the current recommendation, not a permanent model alias. Recheck current OpenAI model guidance when the available model lineup changes.
...
10. Continue through staged reconciliation passes, pausing for Lyle approval between material passes:
   ...
   d. PMR candidates, Agents Docs implications, technical uncertainties, and Pre Spec Implementation Questions.
...
- Provide the top three most likely answers based on the current Implementation Concept, current Pre-Spec, PMR when relevant, and current Agents Docs.
...
- Do not ask open-ended engineering questions when likely answer options can be inferred from Agents Docs and the Pre-Spec.
...
- Do not write to Agents Docs. Agents Docs are read-only for this project.
...
- Agents Docs, for implementation implications and validation questions;
...
8. Use Agents Docs to flag implementation implications and uncertainty, not to override product decisions. When Agents Docs is used, check root AGENTS.md, AGENTS-CHANGELOG.md, and the relevant focused doc's Last updated line before relying on the information.
...
4. an Agents Docs implementation implication;
...
11. Add Agents Docs / implementation implications as concept-level validation items or Pre Spec Implementation Questions.
...
Purpose:
Implementation Concepts are the bridge between Feature Lab product discussion and the eventual Claude-ingestible implementation spec.
...
Definition:
An Implementation Concept is a feature-specific build concept that must be understood as a coherent unit in order for Claude or engineering to produce an accurate implementation plan or implementation output.
...
A good Implementation Concept should name the specific feature concept Claude must understand.
...
- a concept that will likely become a heading in the final Claude-ingestible implementation spec;
...
- a concept that Claude would likely need to reason about separately when producing an implementation plan.
...
Source Basis:
Transcript, Pre-Spec section, Lyle instruction, PMR reference, Agents Docs reference, mockup, related spec, or engineering feedback that introduced or changed the concept.
...
Known or likely surfaces involved, using current Agents Docs terminology where possible.
...
- current Agents Docs guidance, when implementation terminology or likely surfaces are needed;
...
- the name would confuse Claude or engineering during implementation planning.
...
3. Check the PMR and current Agents Docs where relevant.
...
When the feature is ready for a Spec Sheet Draft, the confirmed Implementation Concepts and their draft implementation instructions should become the primary source for Chapter 2: Claude-Ingestible Implementation Spec.
...
Purpose:
Pre Spec Implementation Questions are the running engineering-facing question list inside the Feature Working Model / Pre-Spec. They capture implementation uncertainties that arise while drafting concept-level implementation instructions from the current Implementation Concepts list. They may originate during transcript ingestion, PMR review, Agents Docs review, Feature Spec Sheet Instructions gap review, or pre-spec drafting, but should not consume Feature Lab agenda time unless they require a product decision.
...
3. Agents Docs, for likely WDE/Usked 2.0 implementation surfaces and technical architecture implications. Use root AGENTS.md as the navigation hub, check AGENTS-CHANGELOG.md for freshness/change history, and open the relevant focused docs under documents/ as needed.
...
4. Agents Docs suggests multiple possible implementation paths or leaves uncertainty about the correct implementation surface;
...
3. Question.
4. Why it matters.
5. Likely reference area: current Pre-Spec, PMR, Agents Docs, Feature Spec Sheet Instructions, or any combination.
...
10. Agents Docs / implementation implications have been reviewed enough to avoid vague or impossible implementation language, and engineering-facing uncertainties have been captured in Pre Spec Implementation Questions where applicable.
...
23. Relationship to Project Settings, Feature Spec Sheet Instructions, PMR, and Agents Docs
Project Settings:
...
Agents Docs:
Use Agents Docs to identify likely WDE/Usked 2.0 implementation surfaces and uncertainty. Agents Docs informs implementation clarity but does not override Feature Lab product decisions. Agents Docs is a living folder-based corpus; use root AGENTS.md as the navigation hub, AGENTS-CHANGELOG.md as the freshness/change-history anchor, and relevant focused docs under documents/ for the task. Do not list every Agents Docs file in a Pre-Spec unless a specific focused doc is directly implicated by the feature.
...
25. Closeout Status
Current. This document includes Implementation Concepts, draft implementation instruction rules, Pre Spec Implementation Questions, Feature Spec Sheet Instructions alignment, ordinary retrospective reconstruction guidance, consolidated pre-historic reconstruction guidance, model-selection guidance for large-corpus ingestion, and Agents Docs source and freshness alignment. The header metadata is the authority for this document's current update status.
